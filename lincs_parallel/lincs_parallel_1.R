...
dir<-dir(path = "~/LINCS project/LINCS specific scripts/sq_gctx_experiments")
...
for (i in 1:length(current_samples)){
	...
	if (filename %in% dir){
	    ...
	    temp<-tryCatch(geo2fingerprint(
    	    ...
    	    progressBar = FALSE
    	# Do nothing if error in an experiment occurs [add handlers later]*
